...
async function reps(){
  shell('DocuSign Documents','Completed envelopes grouped by internal rep','<div class="toolbar"><input id="search" placeholder="Search reps by name or email"><select id="sort"><option value="recent">Recent activity</option><option value="name">Rep name</option><option value="count">Envelope count</option></select></div><div id="results" class="grid"></div>');
  const load=async()=>{const query=new URLSearchParams({search:document.querySelector('#search').value,sort:document.querySelector('#sort').value});const {reps}=await api('/api/reps?'+query);document.querySelector('#results').innerHTML=reps.length?reps.map(rep=>'<a class="card" href="/documents/reps/'+encodeURIComponent(rep.repId)+'"><div class="card-row"><div><h2>'+esc(rep.name)+'</h2><div class="meta">'+esc(rep.email||'External or unknown sender')+'</div><div class="meta">Last activity: '+esc(date(rep.latestCompletedAt))+'</div></div><div class="count">'+rep.completedEnvelopeCount+' completed envelope'+(rep.completedEnvelopeCount===1?'':'s')+'</div></div></a>').join(''):'<div class="empty">No completed envelopes found.</div>'};
...
async function repPage(repId){
  ...
  shell(data.rep.name,data.rep.email||'Unassigned sender group','<div class="toolbar"><input id="search" placeholder="Search document, envelope ID, or date"><select id="sort"><option value="newest">Newest completed</option><option value="oldest">Oldest completed</option><option value="name">Document name</option></select></div><div id="results" class="grid"></div>','/documents');
...
async function envelopePage(envelopeId){
  ...
  shell(env.rep.name,(env.rep.email||'Unassigned sender')+' · Completed '+date(env.completedAt),'<div class="card"><div class="meta">Envelope ID</div><code>'+esc(env.envelopeId)+'</code></div><section class="documents"><h2>Documents</h2><div class="grid" style="margin-top:12px">'+(docs||'<div class="empty">No documents found.</div>')+'</div></section>','/documents/reps/'+encodeURIComponent(env.rep.repId));
}
