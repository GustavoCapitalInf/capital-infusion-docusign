...
export function contractDistribution(reps = []) {
  ...
  return {
    ...
    'W-2': tracked.filter((rep) => rep.contract.contractType === 'W-2').length,
    1099: tracked.filter((rep) => rep.contract.contractType === '1099').length,
    tier1: tier(1),
    tier2: tier(2),
    tier3: tier(3),
  };
...
<head>
  ...
  <style>
    :root{color-scheme:light;--ink:#171a1f;--muted:#7b818f;--faint:#9aa0ac;--line:#f0f1f5;--line-strong:#eceef2;--paper:#fff;--canvas:#e6e9ee;--soft:#fafbfc;--neutral:#f4f5f8;--neutral-ink:#3d434f;--blue:#2f6bff;--blue-soft:#eef4ff;--green:#0f8f5f;--green-soft:#e9f8f1;--amber:#96700a;--amber-soft:#fff8e3;--orange:#d1620f;--orange-soft:#fff1e8;--red:#e4574e;--red-soft:#ffeeed;--brand:#0f3d2e;--panel:#1c2033;--panel-inner:#232840;--panel-muted:#9aa2bd;--shadow-sm:0 1px 2px rgb(23 26 31/.04);--shadow-md:0 10px 24px rgb(23 26 31/.10);--nav-width:246px;--nav-rail:78px}
    *{box-sizing:border-box}html{scroll-behavior:smooth}
    ...
    .app-shell.nav-collapsed .sidebar{width:var(--nav-rail)}
    .app-shell.nav-collapsed .nav-text,.app-shell.nav-collapsed .nav-count,.app-shell.nav-collapsed .nav-dot,.app-shell.nav-collapsed .nav-section,.app-shell.nav-collapsed .brand-name,.app-shell.nav-collapsed #nav-collapse{display:none}
    .app-shell:not(.nav-collapsed) #nav-expand{display:none}
    .brand{display:flex;align-items:center;gap:11px;height:44px;padding-left:5px;margin-bottom:14px}
    ...
    .nav-toggle{margin-left:auto;display:grid;place-items:center;width:26px;height:26px;border:0;border-radius:8px;background:var(--neutral);color:var(--muted);cursor:pointer}
    .nav-toggle:hover{background:var(--line-strong);color:var(--ink)}
    #nav-expand{display:flex;align-items:center;justify-content:center;width:100%;height:42px;margin:4px 0 0;border:0;border-radius:13px;background:transparent;color:var(--faint);cursor:pointer}
    #nav-expand:hover{background:#f6f7f9;color:var(--ink)}
    ...
    .nav-dot{margin-left:auto;width:7px;height:7px;border-radius:50%;background:#ff8a3d}
    .nav-spacer{flex:1;min-height:26px}
    ...
    .page-header>div{min-width:0}.page-title{margin:0;font-size:26px;font-weight:800;letter-spacing:-.03em;overflow-wrap:anywhere}
    ...
    .type-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,280px),1fr));gap:16px;margin-top:16px}
    .type-card{display:block;width:100%;padding:0;border:0;border-radius:20px;overflow:hidden;text-align:left;cursor:pointer;transition:transform .16s ease}
    .type-card:hover{transform:translateY(-1px)}
    .type-card-top{display:flex;align-items:center;gap:12px;padding:20px 20px 22px}
    .type-avatars{display:flex}
    .type-avatars .avatar{width:38px;height:38px;border-radius:50%;font-size:11px}
    .type-avatars .avatar+.avatar{margin-left:-13px}
    .type-more{display:grid;place-items:center;width:38px;height:38px;margin-left:-13px;border-radius:50%;font-size:11px;font-weight:800}
    .type-card-foot{display:flex;align-items:center;gap:11px;padding:16px 20px}
    .type-card-foot svg{flex:none;width:22px;height:22px}
    .type-name{font-size:19px;font-weight:800;letter-spacing:-.02em}
    .type-count{margin-left:auto;padding:5px 12px;border-radius:999px;background:#fff;font-size:12px;font-weight:700}
    .type-empty{font-size:12.5px;font-weight:600;opacity:.75}
    ...
    .panel-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:16px;margin-top:26px}
    .panel{border:1px solid var(--line);border-radius:20px;background:var(--paper);overflow:hidden}
    .panel+.panel{margin-top:16px}.panel.spaced{margin-top:22px}.tab-panel>.data-list,.tab-panel>.empty-state{margin-top:18px}
    ...
    @media(max-width:960px){.sidebar{width:var(--nav-rail)}.nav-text,.nav-count,.nav-dot,.nav-section,.brand-name,#nav-collapse,#nav-expand{display:none}}
    ...
    <div class="app-frame">
      <nav class="sidebar" aria-label="Main">
        <div class="brand">
          ...
          <button class="nav-toggle" id="nav-collapse" type="button" aria-label="Collapse navigation" aria-controls="app-shell" aria-expanded="true"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 6l-6 6 6 6"/></svg></button>
        </div>
        ...
        <a class="nav-item" id="nav-reps" href="/documents#representatives"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="9" cy="8" r="3.2"/><path d="M3.5 19c0-3 2.5-4.6 5.5-4.6s5.5 1.6 5.5 4.6"/><path d="M16.5 6.6a3 3 0 010 5.4M18 19c0-1.9-.6-3.3-1.8-4.2"/></svg><span class="nav-text">Representatives</span></a>
        <a class="nav-item" id="nav-attention" href="/documents#needs-attention"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 8v4.5M12 16h.01"/></svg><span class="nav-text">Needs Attention</span><span class="nav-dot" id="nav-attention-dot" hidden></span></a>
        <a class="nav-item" id="nav-renewals" href="/documents#upcoming-renewals"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 12a8 8 0 11-2.4-5.7"/><path d="M20 4v4h-4"/></svg><span class="nav-text">Renewals</span></a>
        <div class="nav-spacer"></div>
        <button class="nav-toggle" id="nav-expand" type="button" aria-label="Expand navigation" aria-controls="app-shell" aria-expanded="false"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 6l6 6-6 6"/></svg></button>
      </nav>
      <div class="app-main">
        <header class="topbar">
...
function Icon(name){const paths={chart:'<path d="M4 19h4v-7H4zM10 19h4V6h-4zM16 19h4v-4h-4z"/>',alert:'<circle cx="12" cy="12" r="8.5"/><path d="M12 8v4.5M12 16h.01"/>',clock:'<circle cx="12" cy="12" r="8.5"/><path d="M12 7.5V12l3 2"/>',refresh:'<path d="M20 12a8 8 0 11-2.4-5.7"/><path d="M20 4v4h-4"/>',doc:'<path d="M7 4h7l4 4v12H7z"/><path d="M14 4v4h4"/>',folder:'<path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z"/>',check:'<path d="M4 17l6-6 4 4 6-7"/>',search:'<circle cx="11" cy="11" r="7"/><path d="M20 20l-4-4"/>'};return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'+(paths[name]||paths.search)+'</svg>'}
...
function DataRow(options){return '<a class="data-row'+(options.className?' '+options.className:'')+'" href="'+options.href+'" aria-label="'+esc(options.ariaLabel)+'">'+options.main+(options.meta||'')+(options.tail||'')+(options.chevron===false?'':'<span class="chevron" aria-hidden="true">›</span>')+'</a>'}
...
function trackedReps(reps){return reps.filter(rep=>rep.contract&&rep.contract.currentContract&&rep.contract.currentTier)}
function FilterPills(reps){const summary=dashboardSummary(reps);const distribution=contractDistribution(reps);const pill=(type,count,label)=>'<button class="pill" type="button" role="tab" aria-selected="'+(type==='all'?'true':'false')+'" aria-controls="rep-results" data-contract-type="'+type+'"><strong>'+count+'</strong> <span>'+esc(label)+'</span></button>';const stat=(count,label)=>'<span class="pill static"><strong>'+count+'</strong> <span>'+esc(label)+'</span></span>';return '<div class="filter-pills" role="tablist" aria-label="Representative contract type">'+pill('all',distribution.tracked,'All contracts')+pill('W-2',distribution['W-2'],'W-2')+pill('1099',distribution['1099'],'1099')+stat(summary.renewalsDueSoon,'Renewals due')+stat(summary.expired,'Expired')+'</div>'}
function TypeCard(type,reps){const theme=type==='W-2'?{bg:'#ffc629',shadow:'0 10px 26px rgb(255 198 41/.28)',ink:'#2e2300',foot:'rgb(255 255 255/.42)',avatar:'background:#fff3cd;color:#8a6a00;border:2.5px solid #ffc629',pill:'#8a6a00',icon:'#3a2c00'}:{bg:'#7b61ff',shadow:'0 10px 26px rgb(123 97 255/.28)',ink:'#fff',foot:'rgb(255 255 255/.28)',avatar:'background:#ded7ff;color:#3f2ba8;border:2.5px solid #7b61ff',pill:'#5b41d6',icon:'#fff'};const matching=trackedReps(reps).filter(rep=>rep.contract.contractType===type);const shown=matching.slice(0,3).map(rep=>'<span class="avatar" style="'+theme.avatar+'" aria-hidden="true">'+esc(avatarInitials(rep.name))+'</span>').join('');const overflow=matching.length>3?'<span class="type-more" style="'+theme.avatar+'" aria-hidden="true">+'+(matching.length-3)+'</span>':'';const stack=matching.length?'<span class="type-avatars">'+shown+overflow+'</span>':'<span class="type-empty" style="color:'+theme.ink+'">No representatives yet</span>';return '<button class="type-card" type="button" data-type-card="'+type+'" aria-pressed="false" style="background:'+theme.bg+';box-shadow:'+theme.shadow+'" aria-label="Filter representatives with '+type+' contracts"><span class="type-card-top">'+stack+'</span><span class="type-card-foot" style="background:'+theme.foot+'"><span style="color:'+theme.icon+'" aria-hidden="true">'+Icon('folder')+'</span><span class="type-name" style="color:'+theme.ink+'">'+type+' Contracts</span><span class="type-count" style="color:'+theme.pill+'">'+plural(matching.length,'rep')+'</span></span></button>'}
function tierCards(reps){const distribution=contractDistribution(reps);return '<div class="stat-grid" role="group" aria-label="Contract tier summary">'+StatCard('Tier 1',distribution.tier1,'Initial term','chart','',true)+StatCard('Tier 2',distribution.tier2,'First renewal','chart','green')+StatCard('Tier 3',distribution.tier3,'Long term','chart','violet')+StatCard('Expired',distribution.expired,'Renewal required','alert','red',true)+'</div>'}
...
function renderOverviewPanel(reps){const panel=document.querySelector('#overview-panel');const distribution=contractDistribution(reps);const summary=dashboardSummary(reps);const attention=attentionContracts(reps).length;const upcoming=upcomingContracts(reps).length;panel.innerHTML='<div class="overview-head"><h2>Contract Overview</h2><span class="overview-chip">'+summary.expired+' expired</span></div>'+DonutRing(distribution)+'<div class="overview-split"><div class="overview-tile"><div class="overview-tile-label">Total Active</div><div class="overview-tile-value">'+summary.activeReps+'</div></div><div class="overview-tile"><div class="overview-tile-label">Renewals Due</div><div class="overview-tile-value">'+summary.renewalsDueSoon+'</div></div></div><div class="overview-block">'+OverviewLine('#7b61ff','#fff','doc','1099 Contracts','Independent contractors',distribution['1099'])+OverviewLine('#ffc629','#3a2c00','doc','W-2 Contracts','Employed representatives',distribution['W-2'])+OverviewLine('#4fd1c5','#0c3b37','refresh','Renewals','Expiring in 31–90 days',upcoming)+OverviewLine('#ff6b6b','#fff','alert','Needs Attention','Expired or under 30 days',attention)+'</div><div class="overview-legend"><div class="overview-legend-title">Tier distribution</div><div class="overview-legend-list"><div class="overview-legend-row"><span style="background:#2f8bff"></span><span>Tier 1</span><strong>'+distribution.tier1+'</strong></div><div class="overview-legend-row"><span style="background:#4fd1c5"></span><span>Tier 2</span><strong>'+distribution.tier2+'</strong></div><div class="overview-legend-row"><span style="background:#7b61ff"></span><span>Tier 3</span><strong>'+distribution.tier3+'</strong></div><div class="overview-legend-row"><span style="background:#ff6b6b"></span><span>Expired</span><strong>'+distribution.expired+'</strong></div></div></div>';panel.hidden=false}
...
function renewalPanels(reps){const attention=attentionContracts(reps);const upcoming=upcomingContracts(reps);const attentionPanel=Panel({id:'needs-attention',header:SectionHeader('Needs Attention','Expired contracts and renewals due within 30 days.'),body:attention.length?'<div class="attention-list">'+attention.map(rep=>AttentionRow(rep,false)).join('')+'</div>':EmptyState('No contracts currently require attention.')});const upcomingPanel=Panel({id:'upcoming-renewals',header:SectionHeader('Upcoming Renewals','Active contracts expiring in 31–90 days.'),body:upcoming.length?'<div class="attention-list">'+upcoming.map(rep=>AttentionRow(rep,true)).join('')+'</div>':EmptyState('No upcoming renewals.')});return '<div class="panel-grid">'+attentionPanel+upcomingPanel+'</div>'}
...
function renderRepResults(reps){document.querySelector('#rep-results').innerHTML=reps.length?'<div class="data-list">'+reps.map(RepRow).join('')+'</div>':EmptyState('No representatives found.',true)}
function updateNavCounts(reps){const attention=attentionContracts(reps).length;const badge=document.querySelector('#nav-attention-count');const dot=document.querySelector('#nav-attention-dot');badge.textContent=attention;badge.hidden=attention===0;dot.hidden=attention===0}
async function dashboardPage(){const initial=await api('/api/reps?sort=recent');let contractType='all';document.querySelector('#topbar-search').hidden=false;const sortTools='<div class="toolbar">'+SelectControl('rep-sort','Sort representatives',[{value:'recent',label:'Recent Activity'},{value:'name',label:'Name'},{value:'count',label:'Agreement Count'},{value:'expiration',label:'Contract Expiration'}])+'</div>';const activeMeta='<span class="active-pill"><span class="active-pill-mark" aria-hidden="true">'+Icon('check')+'</span>'+dashboardSummary(initial.reps).activeReps+' active</span>';app.innerHTML=PageHeader({title:'Contract Management',description:'Manage representative agreements, contract status, and renewals.',meta:activeMeta})+FilterPills(initial.reps)+'<div class="type-grid">'+TypeCard('W-2',initial.reps)+TypeCard('1099',initial.reps)+'</div>'+tierCards(initial.reps)+'<div class="section-header" id="representatives"><div><h2 class="section-title">Representatives</h2><p class="section-description">Review contract status and completed agreements.</p></div>'+sortTools+'</div><div id="rep-results" role="tabpanel"></div>'+renewalPanels(initial.reps);renderRepResults(initial.reps);renderOverviewPanel(initial.reps);updateNavCounts(initial.reps);const load=async()=>{const query=new URLSearchParams({search:document.querySelector('#rep-search').value,sort:document.querySelector('#rep-sort').value});if(contractType!=='all')query.set('contractType',contractType);renderRepResults((await api('/api/reps?'+query)).reps)};const setContractType=value=>{contractType=value;for(const item of document.querySelectorAll('[data-contract-type]'))item.setAttribute('aria-selected',String(item.dataset.contractType===value));for(const card of document.querySelectorAll('[data-type-card]'))card.setAttribute('aria-pressed',String(card.dataset.typeCard===value));load()};for(const tab of document.querySelectorAll('[data-contract-type]'))tab.addEventListener('click',()=>setContractType(tab.dataset.contractType));for(const card of document.querySelectorAll('[data-type-card]'))card.addEventListener('click',()=>setContractType(card.dataset.typeCard===contractType?'all':card.dataset.typeCard));document.querySelector('#rep-search').addEventListener('input',load);document.querySelector('#rep-sort').addEventListener('change',load);scrollToHash()}
...
function initShell(){const shell=document.querySelector('#app-shell');const setNav=collapsed=>{shell.classList.toggle('nav-collapsed',collapsed);document.querySelector('#nav-collapse').setAttribute('aria-expanded',String(!collapsed));document.querySelector('#nav-expand').setAttribute('aria-expanded',String(!collapsed))};document.querySelector('#nav-collapse').addEventListener('click',()=>setNav(true));document.querySelector('#nav-expand').addEventListener('click',()=>setNav(false));const current=location.pathname==='/documents'||location.pathname==='/documents/'?'#nav-dashboard':'#nav-reps';document.querySelector(current).setAttribute('aria-current','page')}
